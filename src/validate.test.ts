...
describe('validate function', () => {
  it('when a project convention is kebabCase and all files are correct then we display a success message', async () => {
    const stdout = await execute(
      'npx ts-node src/index folder=./mocks ignore=[README.md, Dockerfile]'
    )
  ...
  it('when a project convention is kebabCase and some files are wrong then we display an error message and kill a process', async () => {
    try {
      ...
      const { stdout } = e
      expect(stdout).toBe(
        `Uuu, some files are not following your project naming convention (kebabCase). Please take a look on below files [ 'mocks/README.md' ]`
      )
    }
  ...
  it('when a project convention is capitalize and some files are wrong then we display an error message and kill a process', async () => {
    try {
      await execute('npx ts-node src/index folder=./mocks type=capitalize ignore=[README.md]')
    } catch (e) {
      const { stdout } = e
      expect(stdout).toBe(
        `Uuu, some files are not following your project naming convention (capitalize). Please take a look on below files [ 'mocks/simple-js-file.js', 'mocks/some-scss-file.sass' ]`
      )
    }
